...
class SUT:
    def __init__(
        ...
        self.model_path = model_path or "meta-llama/Meta-Llama-3.1-405B-Instruct"
    ...
    def process_queries(self):
        ...
        while True:
            ...
            # list[list[int]] of token ids
            input_ids_tensor = [
                self.data_object.input_ids[q.index] for q in qitem
            ]

            tik2 = time.time()
            # Build a list[TokensPrompt] and pass it positionally
            prompts = [
                TokensPrompt(prompt_token_ids=ids)
                for ids in input_ids_tensor
            ]
            outputs = self.model.generate(
                prompts,  # positional, no keyword
...
class SUTServer(SUT):
    ...
    def process_queries(self):
        ...
        while True:
            ...
            # Single sample: build a single TokensPrompt
